...
def CF1895C():
    ...
    for l in s:
        lenl = len(l)
        for lenr in range(lenl & 1, lenl + 1, 2):
            m = lenl + lenr
            suml = sum(map(int, l[: m // 2]))
            sumr = sum(map(int, l[m // 2:]))
            if suml >= sumr:
                ans += cnt[lenr][suml - sumr]
    
    for r in s:
        lenr = len(r)
        for lenl in range(lenr & 1, lenr, 2):
            m = lenr + lenl
            suml = sum(map(int, r[- m // 2: ]))
            sumr = sum(map(int, r[: - m // 2]))
            if suml >= sumr:
