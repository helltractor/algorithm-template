...
def C():
    ...
    inv = [1] * (l + 1)
    inv[l] = pow(fact[l], MOD1 - 2, MOD1)
    for i in range(l - 1, -1, -1):
        inv[i] = (i + 1) * inv[i + 1] % MOD1
    
    def comb(n, r):
        return fact[n] * inv[r] % MOD1 * inv[n - r] % MOD1 if n >= r >= 0 else 0
    ...
    for _ in range(II()):
        ...
        for i in range(cnt // 2 + 1):
            ans = (ans + comb(cnt, 2 * i) * max(1, f(2 * i - 1)) * pow(2, i, MOD1)) % MOD1
        print(ans)
