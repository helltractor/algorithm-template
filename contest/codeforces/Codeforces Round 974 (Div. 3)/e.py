...
def get_shortest_path(dct, src, initial=0, arr=set()):
    ...
    while h:
        ...
        i = u // 2
        t = u % 2
        if not t and i in arr:
            heappush(h, (d, 2 * i + 1))
        for j, w in dct[i]:
            heappush(h, (d + (w // 2 if t else w), 2 * j + t))
...
def E():
    for _ in range(II()):
        ...
        for _ in range(m):
            ...
            g[u].append((v, w))
            g[v].append((u, w))
        dis1 = get_shortest_path(g, 1, 0, a)
        disn = get_shortest_path(g, n, 0, a)
        ans = inf
        for i in range(1, n + 1):
            ans = fmin(ans, max(dis1[i], disn[i]))
