    ...
    for _ in range(II()):
        ...
        while q:
            for _ in range(len(q)):
                x, fa, even, odd = q.popleft()
                for y in g[x]:
                    if y == fa: continue
                    if depth % 2 == 0:
                        ans[y] = fmax(ans[y], odd + a[y])
                        q.append([y, x, even - a[y], fmax(a[y], odd + a[y])])
                    else:
                        ans[y] = fmax(ans[y], even + a[y])
                        q.append([y, x, fmax(a[y], even + a[y]), odd - a[y]])
            depth += 1
        print(*ans)
